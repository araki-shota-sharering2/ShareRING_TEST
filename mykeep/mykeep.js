document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function fetchKeeps(page = 1) {
        try {
            ...
            if (response.ok) {
                ...
                console.error("Failed to fetch keep data.");
            }
        } catch (error) {
            ...
            postFrame.innerHTML = `
                <div class="post-content">
                    <div class="post-frame-wrapper">
                        ...
                        <img src="/assets/images/keep/delete.svg" alt="Keep画像" class="keep-image" data-post-id="${post.post_id}">
                    </div>
    ...
    loadMoreButton.addEventListener("click", () => {
        if (!loadMoreButton.classList.contains("disabled")) {
            currentPage++;
            fetchKeeps(currentPage);
        }
    });

    await fetchKeeps(currentPage);
});
